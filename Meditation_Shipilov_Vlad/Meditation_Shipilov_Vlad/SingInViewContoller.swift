...
class SingInViewContoller: UIViewController {
    ...
    @IBAction func singInAction(_ sender: UIButton) {
        ...
        let param: [String: String] = [
            "emali": inputLogin.text!,
            "password": inputPassword.text!]
        AF.request(url, method: .post, parameters: param, encoder: JSONParameterEncoder.default).validate().responseJSON { responce in
            switch response.result{
                ...
                case .failure(let error):
                    let errorJSON = JSON(responce.data)
